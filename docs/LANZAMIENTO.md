...
Marketplace: <MARKETPLACE_URL> · Open VSX (Cursor/VSCodium): <OPENVSX_URL> · Source: https://github.com/TecniartGalicia/handsfree-claude-code
...
Handsfree hace tres cosas, y ninguna más:
...
[Instalar desde el Marketplace](<MARKETPLACE_URL>) · [Open VSX para Cursor / VSCodium](<OPENVSX_URL>) · [Código en GitHub](https://github.com/TecniartGalicia/handsfree-claude-code)
...
**Tagline:** Claude Code without permission prompts — using its own settings, not hacks.
...
## 5 · Métricas y revisión a 60 días

- **Instalaciones**: Marketplace → Manage → Reports (acumuladas y por día); Open VSX → página de la extensión (descargas).
- **Uso**: no hay telemetría a propósito. La señal de uso real serán issues, Discussions y reseñas.
- **Pro**: panel de Polar (ventas, activaciones por clave, reembolsos).
- **Criterio de decisión** (anotado en el análisis inicial): >2.500 instalaciones o >30 ventas en 60 días → seguir invirtiendo (más checks del Doctor, perfiles de equipo); entre 800 y 2.500 → mantenimiento; <800 → congelar y dejarla como escaparate.
- **Mantenimiento mínimo**: aceptar los PRs de dependabot que pasen la CI (majors de TypeScript/@types/node se ignoran a propósito; `@types/vscode` fijado a 1.95); revisar cada release grande de Claude Code por si cambia una clave (el Doctor avisa: `ext.contract`).
