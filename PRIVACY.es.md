...
Qué escribe: los cuatro ajustes descritos en el README, y copias de seguridad de tu fichero de configuración de Claude en `~/.claude/backups/handsfree/` (se conservan las 10 más recientes). Las funciones Pro escriben además, por comando explícito tuyo: reglas `permissions.ask` / `permissions.deny` en ese mismo fichero de usuario (guardarraíles), `<repositorio>/.claude/settings.local.json` y una línea en el `.git/info/exclude` de ese repositorio (perfil prudente), y un JSON de perfil donde tú elijas (exportar). Las copias son idénticas a tu configuración y por tanto contienen lo mismo que ella, incluidos los valores de `env`; conservan los permisos del fichero original.
...
**Licencia Pro (opcional, de pago).** Cuando *tú* ejecutas "Pro: introducir clave de licencia", la extensión envía tu clave de licencia, el nombre de este equipo (como etiqueta de la activación) y dos campos no identificativos (sistema operativo, versión de la extensión) a la API de licencias de Polar Software Inc. (`api.polar.sh`), que actúa como merchant of record y proveedor de licencias. Después, cada vez que se ejecuta un comando de Handsfree (o la barra de estado Pro se refresca tras un cambio de configuración) y la última comprobación tiene más de 24 horas, revalida la clave (solo clave e id de activación); sigue funcionando sin conexión 14 días. Los equipos sin licencia guardada nunca hacen esta llamada. Nunca se envía nada de tus ficheros de configuración. La clave se guarda en el almacén secreto de VS Code de este equipo. "Pro: desactivar la licencia" la borra y libera la activación en Polar. Política de privacidad de Polar: https://polar.sh/legal/privacy.
