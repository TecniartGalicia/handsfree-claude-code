...
import { resolveClaudeSettingsPath } from '../vscode/env';
...
export async function ensureStatusBar(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (!item) {
    ...
    const watchers = [
      vscode.workspace.createFileSystemWatcher('**/.claude/settings*.json'),
      // the user settings file lives outside the workspace; watch its directory explicitly
      vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.Uri.file(path.dirname(resolveClaudeSettingsPath())), 'settings.json')),
    ];
    for (const w of watchers) {
      w.onDidChange(() => scheduleRefresh(context));
      w.onDidCreate(() => scheduleRefresh(context));
      w.onDidDelete(() => scheduleRefresh(context));
...
export async function refresh(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    const report = await runDoctor(context, { quiet: true });
    ...
    // "careful" = a Handsfree-owned profile (info severity); a foreign disable is a warning and shows as such
    const careful = report.findings.some((f) => f.id.startsWith('ws.policy:') && f.severity === 'info');
    if (s.errors > 0) {
